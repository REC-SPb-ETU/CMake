...
function(cpack_rpm_generate_package)
  ...
  if(CPACK_RPM_PACKAGE_DEBUG)
     ...
     message("CPackRPM:Debug: CPACK_SOURCE_DIRECTORY            = ${CPACK_SOURCE_DIRECTORY}")
  endif()
  ...
  if(CPACK_RPM_SRC_AND_DEBUGINFO_PACKAGES_GENERATE)
    if(NOT CPACK_RPM_ARCHIVE_PATH)
        ...
        set(ARCHIVE_PATH "${CPACK_RPM_DIRECTORY}/SOURCES/${ARCHIVE_NAME}.tgz")         
            execute_process(
                COMMAND "tar" -C "${CPACK_SOURCE_DIRECTORY}" -czf "${ARCHIVE_PATH}" .
           )
        set(TMP_RPM_SOURCE "Source: ${ARCHIVE_NAME}.tgz")
    else()
        file(RENAME "${CPACK_RPM_ARCHIVE_PATH}" "${CPACK_RPM_DIRECTORY}/SOURCES/${CPACK_RPM_PACKAGE_NAME}-${CPACK_RPM_PACKAGE_VERSION}.tgz")
      ...
      set(TMP_RPM_BUILD "%build \n cmake . -DCMAKE_BUILD_TYPE=RELEASE -DCMAKE_INSTALL_PREFIX=${CPACK_RPM_PACKAGE_PREFIX}\n make %{?_smp_mflags}")
...
")
  ...
  if(RPMBUILD_EXECUTABLE)
   ...
   if(CPACK_RPM_SRC_AND_DEBUGINFO_PACKAGES_GENERATE)
       execute_process(
          COMMAND "${RPMBUILD_EXECUTABLE}" -ba
       ...
       set(NAME_RPM_PACKAGE "${CPACK_RPM_PACKAGE_NAME}-${CPACK_RPM_PACKAGE_VERSION}-${CPACK_RPM_PACKAGE_RELEASE}.${CPACK_RPM_PACKAGE_ARCHITECTURE}.rpm")
       file(RENAME "${CPACK_TOPLEVEL_DIRECTORY}/${CPACK_RPM_PACKAGE_ARCHITECTURE}/${NAME_RPM_PACKAGE}" "${CPACK_TOPLEVEL_DIRECTORY}/${CPACK_OUTPUT_FILE_NAME}")
       
       file(GLOB NAME_SRC_PACKAGE "${CPACK_TOPLEVEL_DIRECTORY}/SRPMS/*.src.rpm")
       file(GLOB NAME_DEBUGINFO_PACKAGE "${CPACK_TOPLEVEL_DIRECTORY}/${CPACK_RPM_PACKAGE_ARCHITECTURE}/*.rpm")
 
       file(COPY "${NAME_DEBUGINFO_PACKAGE}" DESTINATION "${CPACK_OUTPUT_FILE_PREFIX}/")
       file(COPY "${NAME_SRC_PACKAGE}" DESTINATION "${CPACK_OUTPUT_FILE_PREFIX}/")     
       
   else()
       execute_process(
          COMMAND "${RPMBUILD_EXECUTABLE}" -bb
                  --define "_topdir ${CPACK_RPM_DIRECTORY}"
                  --define "_rpmfilename ${CPACK_OUTPUT_FILE_NAME}"
                  --define "_rpmdir ${CPACK_RPM_DIRECTORY}"
